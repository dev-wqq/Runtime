...
/**
 1.当images被加载完成(在程序启动或其他情况下)，runtime需要从中加载classes和categories，
   将classes与superclasses建立关联，categories与parent classes建立关联，然后调用+load方法。
 
 2.runtime可以以任意顺序对classes进行处理，也就是说，runtime可能先于superclasses发现class，
   为了处理这种无序的class加载顺序，runtime建立了一种”pending class”机制。
 
 3.一个class在image中第一次被发现时被视为”unconnected”，它被存储在unconected_class_hash中。
   如果该class的所有superclass都存在并且已经被”connected”，这个新的class可以被connected进他
   的superclasses并被移到class_hash中以便被使用。否则该class会一直被存储在unconnected_class_hash
   中知道superclasses完成connecting。
 
 4.image mapping并”不是当前线程安全”的操作，它会在一些情况下被重新执行那个：superclass找到了
   一些引起ZeroLink的原因时回去加载另一个image，或者调用+load的方法时dyld会去加载另一个image。
 
 image mapping 顺序为：
    1.读取images中所有的classes
    2.读取images中所有的categories
    3.对所有classes建立connection
    4.处理selector和class的引用计数
    5.修复images中所有protocol对象
    6.调用+load方法
 */
/***********************************************************************
