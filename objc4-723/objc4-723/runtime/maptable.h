...
/**
 对于Mac/iOS系统执行程序时，首先从Mach-O文件中读取framework和应用中定义的类信息，然后对这些类建立connection，最
 终会存储在一个全局的(extern)NSMapTable结构中，名称为class_hash，等待代码运行时去使用已经建立connection的类。
 
 从images中读取classes后需要构建classes之间的关系网，而构建这一关系网的基础结构是NXMapTable和NXHashTable，
 熟悉这两个结构及其操作函数有助于我们对构建classes之间connection关系部分代码的理解。
 */
/**
 * 下面是hash结构体
 * prototype  :hash函数表
 * count      :hash结构已经有的key个数
 * nbBucketsMinusOne  :hash表的容量大小,随count增多而改变
